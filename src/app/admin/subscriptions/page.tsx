...
export default async function AdminSubscriptionsPage() {
  ...
  const { data } = await supabase
    ...
    .select(
      "id, profile_id, label, items, frequency, status, next_run_date, created_at, last_run_at, profiles(company_name, full_name, email)"
    )
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-6 sm:mb-8">
        All customer subscription schedules. Automation is not yet wired —
        review &ldquo;due soon&rdquo; rows and fulfill them manually until the
        cron is enabled.
      </p>
